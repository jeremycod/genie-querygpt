...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        Commands::Plan { prompt, yes, max_attempts, explain, openai } => {
...
async fn handle_openai_demo(prompt: String) -> anyhow::Result<()> {
    ...
    let client = OpenAIClient::from_env()
        .map_err(|e| anyhow::anyhow!("OpenAI setup failed: {}. Set OPENAI_API_KEY environment variable.", e))?;
...
IMPORTANT: Output only valid JSON. No explanations outside the JSON structure."#.to_string(),
            },
            LlmMessage {
                role: LlmRole::User,
